...
\hypertarget{predictors-and-coefficients}{%
\chapter{Predictors and
coefficients}\label{predictors-and-coefficients}}
...
\hypertarget{linear-model-spaces}{%
\chapter{Linear model spaces}\label{linear-model-spaces}}
...
\chapter{\texorpdfstring{Analysis of variance and
...
{\centering \includegraphics{special-cases_files/figure-pdf/unnamed-chunk-1-1.pdf}
...
\hypertarget{collinearity-and-adjustment}{%
\chapter{Collinearity and
adjustment}\label{collinearity-and-adjustment}}
...
\chapter{Building blocks for linear model
...
\chapter{Confidence and prediction
...
\subsubsection{Sandwich standard
errors}\label{sandwich-standard-errors}}
...
\subsection{Group-correlated errors}\label{group-correlated-errors}}
...
\subsection{Autocorrelated errors}\label{autocorrelated-errors}}
...
\chapter{Origins, consequences, diagnostics, and overview of
...
to outliers; see Section Chapter~\ref{sec-robust-estimation}.

\hypertarget{sec-asymptotic-methods}{%
\chapter{Asymptotic methods for heteroskedastic and correlated
...
\subsubsection{Standard errors via the residual
...
\subsubsection{Hypothesis testing via the residual
...
{\centering \includegraphics{r-demo-part-3_files/figure-pdf/unnamed-chunk-2-1.pdf}
...
{\centering \includegraphics{r-demo-part-3_files/figure-pdf/unnamed-chunk-5-1.pdf}
...
{\centering \includegraphics{r-demo-part-3_files/figure-pdf/unnamed-chunk-6-1.pdf}
...
{\centering \includegraphics{r-demo-part-3_files/figure-pdf/unnamed-chunk-7-1.pdf}
...
\subsection{Moments of exponential dispersion model
...
\subsection{Relationships among the mean, variance, and natural
...
\subsubsection{Relationship between the mean and the natural
...
\subsubsection{Relationship between the mean and
...
\subsection{The unit deviance}\label{the-unit-deviance}}
...
See Figure Figure~\ref{fig-poisson-unit-deviance} for an example of the
shape of this function.

\begin{figure}

{\centering \includegraphics[width=0.5\textwidth,height=\textheight]{figures/poisson-unit-deviance.pdf}
...
\subsection{Small-dispersion approximations to an
...
\subsubsection{The normal
approximation}\label{the-normal-approximation}}

\hypertarget{the-approximation}{%
\paragraph{The approximation}\label{the-approximation}}
...
\paragraph{Approximation accuracy}\label{approximation-accuracy}}
...
\subsubsection{The saddlepoint
approximation}\label{the-saddlepoint-approximation}}

\hypertarget{the-approximation-1}{%
\paragraph{The approximation}\label{the-approximation-1}}
...
\paragraph{Approximation accuracy}\label{approximation-accuracy-1}}
...
\chapter{Generalized linear models and examples}\label{sec-glm-def}}
...
\subsubsection{Example: Weighted linear regression
...
\subsubsection{Example: Ungrouped logistic regression
...
\subsubsection{Example: Grouped logistic regression
...
\subsubsection{Example: Poisson regression
...
\chapter{Parameter estimation in GLMs}\label{sec-glm-max-lik}}

\hypertarget{sec-glm-likelihood}{%
\subsection{The GLM likelihood, score, and Fisher
...
\subsection{\texorpdfstring{Maximum likelihood estimation of
...
\subsubsection{Log-concavity of GLM
...
\subsubsection{Newton-Raphson}\label{sec-newton-raphson}}
...
\subsubsection{\texorpdfstring{Estimation of \(\phi_0\) and GLM
...
\subsubsection{Inferential goals}\label{sec-inferential-goals}}
...
\paragraph{Hypothesis testing}\label{sec-hypothesis-testing}}
...
\item
  ...
  the GLM Equation~\ref{eq-glm-def} is correctly specified. Consider the
...
\paragraph{Confidence interval/region
...
\subsubsection{Inferential tools}\label{sec-inferential-tools}}
...
to testing and estimating coefficients in GLMs Equation~\ref{eq-glm-def}
with a fixed number of parameters as the sample size grows, but not to
testing goodness of fit. Indeed, goodness-of-fit tests refer to the
saturated model Equation~\ref{eq-saturated-model}, whose number of
parameters grows with \(n\). Small-dispersion asymptotics, on the other
hand, apply to goodness-of-fit testing.
...
well). See Figure Figure~\ref{fig-trinity-comparison}.
...
Equation~\ref{eq-glm-fisher-info}. This approximation can be justified
via \emph{large-sample asymptotics} or \emph{small-dispersion
asymptotics}. Wald inference is easy to carry out, and for this reason,
it is considered the default type of inference. However, as we will see
in Unit 5, it also tends to be the least accurate in small samples.
Furthermore, Wald tests are usually not applied for testing goodness of
fit.

\hypertarget{sec-wald-test-single-coeff}{%
\subsubsection{\texorpdfstring{Wald test for \(\beta_j = \beta_j^0\)
(known
...
Based on the Wald approximation Equation~\ref{eq-wald-approximation},
...
\subsubsection{\texorpdfstring{Wald test for
...
\subsubsection{\texorpdfstring{Wald confidence interval for \(\beta_j\)
(known
...
\subsubsection{\texorpdfstring{Wald confidence region for
...
\subsubsection{\texorpdfstring{Wald confidence intervals for \(\eta_i\)
and \(\mu_i\) (known
...
\subsubsection{\texorpdfstring{Wald inference when \(\phi_0\) is
...
\{\#sec-likelihood-ratio-inference\}

\hypertarget{sec-likelihood-ratio-coeff-known}{%
\subsubsection{\texorpdfstring{Testing one or more coefficients
(\(\phi_0\)
...
Equation~\ref{eq-glm-likelihood-via-deviance}). Let
...
the approximation Equation~\ref{eq-likelihood-ratio-approximation}, we
can define the likelihood ratio test:
...
\subsubsection{Confidence interval for a single
...
\subsubsection{\texorpdfstring{Goodness of fit testing (\(\phi_0\)
...
\subsubsection{\texorpdfstring{Likelihood ratio inference for \(\phi_0\)
...
\subsection{Score-based inference}\label{sec-score-inference}}
...
\subsubsection{\texorpdfstring{Testing multiple coefficients (\(\phi_0\)
...
Recalling the expressions Equation~\ref{eq-glm-score} and
Equation~\ref{eq-glm-fisher-info} for the score vector and information
matrix, we have that:
...
\subsubsection{\texorpdfstring{Testing a single coefficient (\(\phi_0\)
...
\subsubsection{\texorpdfstring{Confidence interval for a single
coefficient (\(\phi_0\)
...
\subsubsection{\texorpdfstring{Score test inference for \(\phi_0\)
...
\subsection{Crime data}\label{sec-crime-data}}
...
\subsection{Noisy miner data}\label{sec-noisy-miner-data}}
...
\subsection{Model definition and
interpretation}\label{sec-logistic-model}}
...
\subsubsection{\texorpdfstring{Special case: \(2 \times 2\) contingency
...
\subsection{Logistic regression with case-control
...
\subsection{Estimation and inference}\label{sec-estimation-inference}}

\hypertarget{sec-score-fisher}{%
\subsubsection{Score and Fisher information}\label{sec-score-fisher}}
...
\hypertarget{sec-wald-inference}{%
\subsubsection{Wald inference}\label{sec-wald-inference}}
...
{\centering \includegraphics[width=0.5\textwidth,height=\textheight]{figures/hauck-donner.pdf}
...
\paragraph{The Bernoulli and binomial
...
\paragraph{Likelihood ratio inference for one or more
...
\subsection{Goodness of fit testing}\label{sec-goodness-of-fit}}
...
\subsection{\texorpdfstring{Example: \(2 \times 2\)
...
model. The score equations \ref{eq-logistic-score-equations} reduce to:
...
Section \textbf{?@sec-score-tests-1}, the score statistic for goodness
of fit is Pearson's \(X^2\) statistic:
...
Note that the independence hypothesis \ref{eq-independence-2} reduces to
the hypothesis \(H_0: \beta_{12} = 0\):
...
\subsection{Hierarchical Poisson
...
\subsection{Negative binomial as exponential dispersion
...
\subsection{Estimation in negative binomial
...
\subsection{Wald inference}\label{wald-inference}}
...
\subsection{Overdispersion in logistic
...
\subsection{Contingency table analysis}\label{sec-contingency-table}}
...
\hypertarget{sec-crime-data}{%
\subsection{Revisiting the crime data, again}\label{sec-crime-data}}
...
\chapter{Introduction to multiple
testing}\label{sec-introduction-multiple-testing}}
...
\subsection{The multiplicity problem}\label{sec-multiplicity-problem}}
...
\subsection{Global testing and multiple
...
goals}\label{subsec-multiple-testing-goals}}
...
\subsubsection{Definitions of Type-I error rate and
...
\subsubsection{Relationship between the FWER and
...
\subsection{Bonferroni global test (Bonferroni, 1936 and Dunn,
1961)}\label{sec-bonferroni-global-test}}

\hypertarget{test-definition-and-validity}{%
\subsubsection{Test definition and
...
\subsubsection{\texorpdfstring{The impact of \(p\)-value
...
\subsection{Fisher combination test (Fisher,
...
\subsection{The Bonferroni procedure for FWER
...
\subsection{The Benjamini-Hochberg procedure for FDR
...
\subsubsection{FDR control under
...
\subsection{Additional topics}\label{sec-additional-topics}}

\hypertarget{sec-weighted-multiple-testing}{%
\subsubsection{Weighted multiple testing
