...
&\equiv N(0, \text{SE}(\widehat \beta_j)^2),
\end{split}
$$
...
Recalling that $U(\boldsymbol{\beta}) = \frac{\partial \ell(\boldsymbol{\beta})}{\partial \boldsymbol{\beta}}$ is the score vector, score test inference is based on the following asymptotic approximation:
$$
[U(\boldsymbol{\widehat{\beta}}^0)]_S \overset{\cdot}{\sim} N(\boldsymbol{0}, \{[\boldsymbol{I}(\boldsymbol{\widehat{\beta}}^0)^{-1}]_{S,S}\}^{-1}),
$$ {#eq-score-approximation-1}
from which it follows that
$$
[U(\boldsymbol{\widehat{\beta}}^0)]_S^T [\boldsymbol{I}(\boldsymbol{\widehat{\beta}}^0)^{-1}]_{S,S} [U(\boldsymbol{\widehat{\beta}}^0)]_S \overset{\cdot}{\sim} \chi^2_{|S|}.
$$ {#eq-score-approximation-2}
This approximation can be justified either by small-dispersion asymptotics or large-sample asymptotics (both based on the central limit theorem). Recalling the expressions ([-@eq-glm-score]) and ([-@eq-glm-fisher-info]) for the score vector and information matrix, we have that:

$$
\begin{split}
&[U(\boldsymbol{\widehat{\beta}}^0)]_S^T [\boldsymbol{I}(\boldsymbol{\widehat{\beta}}^0)^{-1}]_{S,S} [U(\boldsymbol{\widehat{\beta}}^0)]_S = \\
&\quad\frac{1}{\phi_0}(\boldsymbol{y} - \boldsymbol{\widehat{\mu}}^0)^T \boldsymbol{\widehat{W}}^0 \boldsymbol{\widehat{M}}^0 \boldsymbol{X}_{S,*}[(\boldsymbol{X}^T \boldsymbol{\widehat{W}}^0 \boldsymbol{X})^{-1}]_{S,S} \boldsymbol{X}_{S,*}^T \boldsymbol{\widehat{M}}^0 \boldsymbol{\widehat{W}}^0 (\boldsymbol{y} - \boldsymbol{\widehat{\mu}}^0).
\end{split}
$$
...
If $S = \{j\}$, the score test becomes univariate and can be based on the normal approximation ([-@eq-score-approximation-1]), which specializes to
$$
...
We can view goodness of fit testing as testing a hypothesis about the coefficients in a GLM with $\boldsymbol{X} = \boldsymbol{I}_{n \times n}$, which amounts to the saturated model that allows unrestricted means for each observation. Furthermore, the coefficient vector fitted under the null hypothesis amounts to fitting the GLM as usual. Such a test, however, does not exactly fall into the framework of @sec-score-test-multiple-coeff, because the null hypothesis concerns the entire coefficient vector rather than just a subset. To generalize the score test approximation ([-@eq-score-approximation-1]), note first that equation ([-@eq-score-null-estimate]) implies that
$$
...
$$
[U(\boldsymbol{\widehat{\beta}}^0)]_S^T [\boldsymbol{I}(\boldsymbol{\widehat{\beta}}^0)^{-1}]_{S,S} [U(\boldsymbol{\widehat{\beta}}^0)]_S = U(\boldsymbol{\widehat{\beta}}^0)^T \boldsymbol{I}(\boldsymbol{\widehat{\beta}}^0)^{-1} U(\boldsymbol{\widehat{\beta}}^0).
$$
...
$$
Note that the latter formulation does not rely on extracting subsets of $\boldsymbol U$ or $\boldsymbol I$, and the quantity $|S|$ can be interpreted as the difference in the number of parameters in the null and alternative models. This formulation is amenable the goodness of fit setting. We derive that
$$
\begin{split}
