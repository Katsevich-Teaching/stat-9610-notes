...
\begin{tcolorbox}[enhanced jigsaw, opacityback=0, titlerule=0mm, breakable, colframe=quarto-callout-note-color-frame, coltitle=black, rightrule=.15mm, leftrule=.75mm, bottomtitle=1mm, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Note}, colback=white, opacitybacktitle=0.6, arc=.35mm, left=2mm, bottomrule=.15mm, toprule=.15mm, colbacktitle=quarto-callout-note-color!10!white, toptitle=1mm]
...
\begin{tcolorbox}[enhanced jigsaw, opacityback=0, titlerule=0mm, breakable, colframe=quarto-callout-note-color-frame, coltitle=black, rightrule=.15mm, leftrule=.75mm, bottomtitle=1mm, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Random versus fixed effects models}, colback=white, opacitybacktitle=0.6, arc=.35mm, left=2mm, bottomrule=.15mm, toprule=.15mm, colbacktitle=quarto-callout-note-color!10!white, toptitle=1mm]
...
}\label{eq-beta-s-2}\end{equation} Combining statements
(\ref{eq-beta-s-1}) and (\ref{eq-beta-s-2}), we verify the claimed
relationship (\ref{eq-claim-wald-f}). Therefore, we have \[
...
\begin{tcolorbox}[enhanced jigsaw, opacityback=0, breakable, colframe=quarto-callout-note-color-frame, toprule=.15mm, arc=.35mm, left=2mm, bottomrule=.15mm, rightrule=.15mm, colback=white, leftrule=.75mm]
...
\begin{tcolorbox}[enhanced jigsaw, opacityback=0, titlerule=0mm, breakable, colframe=quarto-callout-important-color-frame, coltitle=black, rightrule=.15mm, leftrule=.75mm, bottomtitle=1mm, title=\textcolor{quarto-callout-important-color}{\faExclamation}\hspace{0.5em}{The percentile bootstrap}, colback=white, opacitybacktitle=0.6, arc=.35mm, left=2mm, bottomrule=.15mm, toprule=.15mm, colbacktitle=quarto-callout-important-color!10!white, toptitle=1mm]
...
\begin{tcolorbox}[enhanced jigsaw, opacityback=0, titlerule=0mm, breakable, colframe=quarto-callout-warning-color-frame, coltitle=black, rightrule=.15mm, leftrule=.75mm, bottomtitle=1mm, title=\textcolor{quarto-callout-warning-color}{\faExclamationTriangle}\hspace{0.5em}{Warning}, colback=white, opacitybacktitle=0.6, arc=.35mm, left=2mm, bottomrule=.15mm, toprule=.15mm, colbacktitle=quarto-callout-warning-color!10!white, toptitle=1mm]
...
\(m = y_{++} \sim \text{Poi}(\mu_{++})\); for example, maybe
\(y_{++} = m\) was fixed in advance. Is the Poisson inference proposed
above actually valid in the latter case? In fact, it is! To see this,
let us consider the log likelihoods of the two models:
...
so \(x_1 \perp \!\!\! \perp x_2\) if and only if \(\tilde \beta_1 = 0\).
Due to the equivalence between Poisson and multinomial distributions,
the hypothesis tests and confidence intervals for the log odds ratio
\(\beta_{12}\) (or \(\tilde \beta_1\)) obtained from Poisson and
logistic regressions will be the same.
...
testing for independence between \(x_1\) and \(x_2\), which amounts to a
...
\] Here, \(j = 1, \dots, J-1\), \(k = 1, \dots, K-1\), and
\(l = 1, \dots, L-1\).
...
\end{split}
\] In this representation, there is some multicollinearity among the
predictors. For example, \(\{1(x_{i3 = l})\}_{l = 0}^{L-1}\) are
multicollinear with the intercept, so we remove the \(1(x_{i3} = 0)\).
Next, \(\{1(x_{i1} = j)1(x_{i3} = l)\}_{j = 0}^{J-1}\) are
multicollinear with \(1(x_{i3} = l)\) for each \(l\), so we remove
\(1(x_{i1} = 0)1(x_{i3} = l)\) for each \(l\). Similarly,
\(\{1(x_{i2} = k)1(x_{i3} = l)\}_{k = 0}^{K-1}\) are multicollinear with
\(1(x_{i3} = l)\) for each \(l\), so we remove
\(1(x_{i2} = 0)1(x_{i3} = l)\) for each \(l\). This leaves us with the
model \[
\begin{split}
...
\end{split}
\] Note that the coefficients here are not necessarily the same as the
coefficients in the display above. This formulation recovers the
conditional independence model referenced above.
...
\boldsymbol{W} = \text{diag}\left(\frac{\mu_i}{1 + \gamma \mu_i}\right); \quad \boldsymbol{M} = \text{diag}\left(\frac{1}{\mu_i}\right).
...
nontrivial one for several reasons. In this chapter, we formalize this
task as a \emph{multiple testing problem}, discuss the challenges that
arise, and describe some common methods for addressing these challenges.
...
\(p_1, \dots, p_m\) be the corresponding \(p\)-values.
...
Let us define \[
\mathcal{H}_0 \equiv \{j \in \{1, \dots, m\}: H_{0j} \text{ is true}\}
\] and \[
\mathcal{H}_1 \equiv \{j \in \{1, \dots, m\}: H_{0j} \text{ is false}\}.
\]
...
\begin{definition}[Tukey,
1953]\protect\hypertarget{def-fwer}{}\label{def-fwer}
...
\begin{definition}[Benjamini and Hochberg,
1995]\protect\hypertarget{def-fdr}{}\label{def-fdr}
...
stochastically smaller than \(\chi^2_{2m}\), from which the conclusion
follows.
...
We first present the BH procedure, as reformulated by Storey, Taylor,
and Siegmund (2004). Consider thresholding the \(p\)-values at
...
\begin{equation}\protect\hypertarget{eq-bh-rejection-set}{}{
\hat S \equiv \{j: p_j \leq \widehat t\},
}\label{eq-bh-rejection-set}\end{equation} where
\begin{equation}\protect\hypertarget{eq-bh-threshold}{}{
\widehat t = \max\{t \in [0,1]: \widehat{\text{FDP}}(t) \leq q\}.
}\label{eq-bh-threshold}\end{equation}
...
\textbf{BH procedure (Storey et al.~formulation)}\vspace{2mm}

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  For each \(t \in [0,1]\), compute the estimated FDP using
  (\ref{eq-fdp-estimate}).
\item
  Let \(\hat t\) be the largest threshold \(t\) for which the estimated
  FDP is below \(q\) (\ref{eq-bh-threshold}).
\item
  Reject the null hypotheses \(j\) whose \(p\)-values are below
  \(\hat t\) (\ref{eq-bh-rejection-set}).
\end{enumerate}
...
\textbf{BH procedure (original formulation)}\vspace{2mm}
...
\item
  Let \(\hat k\) be the largest index \(k\) such that
  \(p_{(k)} \leq \frac{k}{m}q\), or 0 if no such \(k\) exists.
\item
  Reject the null hypotheses \(j\) corresponding to the \(\hat k\)
  smallest \(p\)-values.
\end{enumerate}

\end{tcolorbox}

These two formulations are equivalent. Indeed, defining
\(p_{(0)} \equiv 0\) for convenience, note that \[
\begin{split}
...
if the \(p\)-values are independent.
...
We present two proofs of this fact, one due to Benjamini and Yekutieli
(2001) and one due to Storey, Taylor, and Siegmund (2004).

\begin{proof}[Benjamini and Yekutieli, 2001]

As a starting point, note that
\begin{equation}\protect\hypertarget{eq-fdp-decomp}{}{
\begin{split}
...
\end{split}
}\label{eq-fdp-decomp}\end{equation} From here, define
\(\hat S(p_j \rightarrow 0)\) as the rejection set of BH when applied to
the \(p\)-values \((p_1, \dots, p_{j-1}, 0, p_{j+1}, \dots, p_m)\). We
claim that if the \(j\)th hypothesis is the rejected, the size of the
rejection set does not change if the \(j\)th \(p\)-value is set to 0. In
other words, we claim that
\begin{equation}\protect\hypertarget{eq-key-bh-claim}{}{
1\left(p_j \leq \frac{qk}{m}, |\hat S| = k\right) = 1\left(p_j \leq \frac{qk}{m}, |\hat S(p_j \rightarrow 0)| = k\right).
}\label{eq-key-bh-claim}\end{equation} This can be seen by noting that
\(|\hat S| = k\) if and only if \(p_{(k)} \leq qk/m\) and
\(p_{(k')} > qk'/m\) for each \(k' > k\). Therefore, if \(|\hat S| = k\)
and \(p_j \leq qk/m\), then \(p_j\) is among the \(k\) smallest
\(p\)-values, so setting \(p_j\) to zero will preserve the statements
\(p_{(k)} \leq qk/m\) and \(p_{(k')} > qk'/m\), so
\(|\hat S(p_j \rightarrow 0)| = k\). The converse implication can be
established by similar logic. Given the statement
(\ref{eq-key-bh-claim}), we can deduce from equation
(\ref{eq-fdp-decomp}) that \[
\begin{split}
...
\begin{proof}[Storey, Taylor, and Siegmund, 2004]
...
Under some forms of dependence, the BH procedure can be shown to control
FDR at level \(q\). One such form of dependence is a special kind of
positive dependence, defined below:

\begin{definition}[]\protect\hypertarget{def-prds}{}\label{def-prds}

The \(p\)-values \((p_1, \ldots, p_m)\) are said to be positively
regression dependent on a subset
\(\mathcal J \subseteq \{1, \dots, m\}\) (PRDS on \(\mathcal J\)) if for
all \(j \in \mathcal J\) and all non-decreasing sets
\(D \subseteq [0,1]^m\) (sets for which \(z \in D\) and \(z' \geq z\)
coordinatewise imply \(z' \in D\)), the quantity \[
\mathbb P[(p_1, \dots, p_m) \in D \mid p_j = t]
\] is nondecreasing in \(t\).

\end{definition}

One example of positively dependent \(p\)-values are those based on
one-sided tests of \(\mu_j = 0\) in based on
\(\boldsymbol y \sim N(\boldsymbol \mu, \boldsymbol \Sigma)\), where
\(\Sigma_{j1,j2} \geq 0\) for all \(j1\) and \(j2\).

Given this definition, we have the following result:

\begin{proposition}[]\protect\hypertarget{prp-bh-prds}{}\label{prp-bh-prds}

If the \(p\)-values are PRDS on \(\mathcal H_0\), then the BH procedure
controls the FDR at level \(q\).

\end{proposition}

This result is due to Benjamini and Yekutieli (2001). See this paper for
a proof. While there are not much more general conditions under which BH
has been proven to control FDR, numerical simulations have shown that
the BH procedure controls FDR under nearly all dependency structures
(though it is possible to construct contrived counterexamples).

For those wanting a rigorous FDR guarantee regardless of dependence
structure, Benjamini and Yekutieli showed that BH can be run with an FDR
level of \(q/(1 + \frac{1}{2} + \cdots + \frac{1}{m})\). However, this
...
This proof is for the case that \(p_j \sim \text{Unif}[0,1]\) for
\(j \in \mathcal H_0\). Using the relationship (\ref{eq-fdp-decomp}), we
have \[
\begin{split}
...
to 1 by assumption. This completes the proof.

\end{proof}

\hypertarget{sec-adaptive-multiple-testing}{%
\subsection{Adaptive multiple testing
procedures}\label{sec-adaptive-multiple-testing}}

Recall that, under independence, the BH procedure controls Type-I error
at the level \(\tfrac{|\mathcal H_0|}{m}q\). To the extent that
\(|\mathcal H_0| < m\), this makes the BH procedure conservative. If we
had access to the null proportion
\(\pi \equiv \tfrac{|\mathcal H_0|}{m}\), then we could control FDR at
level \(q\) by applying BH targeting a less stringent FDR level of
\(q/\pi\). Since \(\pi\) is usually unknown, Storey et al.~(2004)
proposed the following estimator:
\begin{equation}\protect\hypertarget{eq-storey-pi-hat}{}{
\hat \pi_\lambda \equiv \frac{|\{j: p_j > \lambda\}|}{m(1 - \lambda)},
}\label{eq-storey-pi-hat}\end{equation} where \(\lambda \in (0,1)\) is a
tuning parameter, with default recommended value of \(\lambda = 0.5\).
The motivation for this definition is that \(p\)-values larger than a
threshold \(\lambda\) are mostly from null hypotheses, so we would
expect \[
|\{j: p_j > \lambda\}| \approx |\{j \in \mathcal H_0: p_j > \lambda\}| \approx |\mathcal H_0|\mathbb P[\text{Unif}[0,1] > \lambda] = \pi m (1 - \lambda).
\]

This leads to the Storey-BH procedure, which adapts to the null
proportion:

\begin{tcolorbox}[enhanced jigsaw, opacityback=0, breakable, colframe=quarto-callout-note-color-frame, toprule=.15mm, arc=.35mm, left=2mm, bottomrule=.15mm, rightrule=.15mm, colback=white, leftrule=.75mm]

\textbf{Storey-BH procedure}\vspace{2mm}

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  Estimate the null proportion \(\hat \pi_\lambda\) using equation
  (\ref{eq-storey-pi-hat}).
\item
  Run the BH procedure, targeting nominal level \(q/\hat \pi_\lambda\).
\end{enumerate}

\end{tcolorbox}

Storey et al.~(2004) showed that a small modification of the Storey-BH
procedure controls FDR at level \(q\) under independent \(p\)-values.

\begin{proposition}[]\protect\hypertarget{prp-storey-bh}{}\label{prp-storey-bh}

The Storey-BH procedure with \[
\hat \pi_\lambda^+ = \frac{1 + |\{j: p_j > \lambda\}|}{m(1-\lambda)}
\] instead of \(\hat \pi_\lambda\) controls FDR at level \(q\) under
independent \(p\)-values.

\end{proposition}

Note that the Storey-BH procedure is less resilient to \(p\)-value
dependency than the BH procedure.

\hypertarget{sec-fdx-control}{%
\subsection{False discovery exceedance control}\label{sec-fdx-control}}

Controlling the mean of the FDP (i.e., the FDR), does not guarantee that
the FDP will not substantially exceed the nominal level for a given
realization of the \(p\)-values. To address this, the stricter false
discovery exceedance (FDX) criterion was proposed (Guo and Romano,
2005).

\begin{definition}[]\protect\hypertarget{def-fdx}{}\label{def-fdx}

The \emph{false discovery exceedance} (FDX) of a multiple testing
procedure with respect to threshold \(\gamma \in (0,1)\) is the
probability that the FDP exceeds this threshold: \[
\text{FDX}_\gamma \equiv \mathbb P[\text{FDP}(\hat S) > \gamma].
\] A procedure controls the FDX with threshold \(\gamma\) at level
\(\alpha\) if \(\text{FDX}_\gamma \leq \alpha\).

\end{definition}

A simple way to control the FDX is to apply the BH procedure with a more
stringent FDR level of \(\gamma \alpha\).

\begin{proposition}[]\protect\hypertarget{prp-fdx-bh}{}\label{prp-fdx-bh}

Under independence or PRDS on \(\mathcal H_0\), the BH procedure
targeting FDR level \(\gamma \alpha\) controls the FDX with threshold
\(\gamma\) at level \(\alpha\).
...
\] Given independent or PRDS \(p\)-values,
Proposition~\ref{prp-bh-fdr-control} and Proposition~\ref{prp-bh-prds}
guarantee that \(\text{FDR} \leq \gamma \alpha\), which implies
\(\text{FDX}_\gamma \leq \alpha\).
